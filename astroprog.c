...
unsigned char rab=0b00000001, pos=0, mark, step_r, step_l, i;
...
//Возвращаем диск в стартовое положение;

/*while (PIND.5!=1)
      {
      PORTB=rab;
      rab=rab<<1;
      if (rab==0b00010000) rab=0b00000001;
      delay_ms(1000);
      }*/
...
while (1)
      ...
      mark=(PIND.4<<2|PIND.3<<1|PIND.2)&0b00000111;
      step_r=mark-pos;
      step_l=6-mark+pos;
        if (step_r<=step_l) {
        while (step_r) {  
